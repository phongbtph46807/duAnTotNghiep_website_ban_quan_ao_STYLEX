							...
							<?php if(!empty($cartData) && count($cartData)): ?>
								...
								<?php if($sizeName || $colorName || $textureName): ?>
									<div class="stext-110" style="margin-top:6px; display:flex; gap:6px; flex-wrap:wrap;">
										...
										<?php endif; ?>
										<?php if($textureName): ?>
											<span style="background:#f3f3f3; color:#333; border:1px solid #e5e5e5; border-radius:12px; padding:2px 8px; font-size:12px;">Chất liệu: <?php echo e($textureName); ?></span>
										<?php endif; ?>
									</div>
								<?php endif; ?>
									</td>
...
(function($){
	...
	$(document).on('click', '#update-cart-btn', function(e){
		e.preventDefault();
		$('table.table-shopping-cart tr.table_row.dirty').each(function(){
			var $row = $(this);
			var qty = parseInt($row.find('input.num-product').val(), 10);
			if (isNaN(qty) || qty < 1) qty = 1;
...
<?php echo $__env->make('client.layout.layout', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\laragon\www\duAnTotNghiep_website_ban_quan_ao_STYLEX\resources\views/client/cart/shopping.blade.php ENDPATH**/ ?>
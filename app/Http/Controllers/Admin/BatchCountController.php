...
class BatchCountController extends Controller
{
    public function index()
    {
        ...
        return view('admin.inventory.count-batch.index', compact('requests'));
    }
    ...
    public function create()
    {
        ...
        return view('admin.inventory.count-batch.create', compact('batches'));
    }
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            $existingCount = CountRequest::where('warehouse_id', $validated['warehouse_id'])
                ->where('variant_id', $validated['variant_id'])
                ->where('batch_number', $validated['batch_number'])
                ->whereIn('status', ['PENDING', 'PENDING_ADJUSTMENT'])
                ->first();
            ...
            return redirect()->route('admin.inventory.count-batch.index')
                ->with('success', 'Tạo yêu cầu kiểm kê lô hàng thành công');
        } catch (\Exception $e) {
    ...
    {
        $request = CountRequest::with(['warehouse', 'variant.product', 'variant.color', 'variant.size', 'createdBy', 'countedBy', 'confirmedBy'])->findOrFail($id);
        ...
        return view('admin.inventory.count-batch.show', compact('request', 'stock', 'batchInfo'));
    }
    ...
    public function confirmCount(Request $request, $id)
    {
        $validated = $request->validate([
            ...
            'defect_level' => 'nullable|in:LOW,MEDIUM,HIGH',
        ...
        try {
            ...
            DB::transaction(function () use ($countRequest, $validated, $totalQty) {
                ...
                if ($validated['damaged_qty'] > 0) {
                    \App\Models\DefectAssessment::create([
                        ...
                        'variant_id' => $countRequest->variant_id,
                        'batch_number' => $countRequest->batch_number,
                        'location' => $countRequest->location,
            ...
            return redirect()->route('admin.inventory.count-batch.index')
                ->with('success', 'Xác nhận kiểm kê lô hàng thành công');
        } catch (\Exception $e) {
            return back()->with('error', $e->getMessage());
        }
    }

...
<div class="container-fluid">
    <div class="row mb-3">
        ...
        <div class="col-md-6 text-end">
            <a href="{{ route('admin.inventory.stock-in.create') }}" class="btn btn-success btn-sm">
    ...
    <div class="card shadow-sm">
        <div class="card-header bg-light py-2">
            <h6 class="mb-0">Danh Sách Yêu Cầu Nhập Kho ({{ $batches->total() }})</h6>
        </div>
                    ...
                    <tbody>
                        @forelse ($batches as $request)
                            <tr>
                                ...
                                <td class="text-end">{{ number_format($request->cost_price, 0, ',', '.') }}đ</td>
                                <td>{{ $request->received_date?->format('d/m/Y') }}</td>
                                <td>{{ $request->warehouse->name ?? 'N/A' }}</td>
                                <td>
                                    @if($request->supplier_name)
                                        <div class="small">
                                            <strong>{{ $request->supplier_name }}</strong><br>
                                            @if($request->supplier_contact)
                                                <span class="text-muted">{{ $request->supplier_contact }}</span><br>
                                            @endif
                                            @if($request->invoice_number)
                                                <span class="badge bg-light text-dark">{{ $request->invoice_number }}</span>
                                            @endif
                                        </div>
                                    @else
                                ...
                                <td class="text-end">
                                    @if ($request->status === 'PENDING')
                                        <a href="{{ route('admin.inventory.stock-in.qc', $request->id) }}" class="btn btn-sm btn-info" title="QC Phê Duyệt">
                                            <i class="bx bx-check"></i>
                                        </a>
                                    @elseif ($request->status === 'QC_PASSED')
                                        <form action="{{ route('admin.inventory.stock-in.confirm', $request->id) }}" method="POST" style="display:inline;">
                                            @csrf

        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header">
                    <h5 class="card-title mb-0">Sửa Chất Liệu: {{ $texture->name }}</h5>
                </div>
                <div class="card-body">
                    <form action="{{ route('admin.textures.update', $texture) }}" method="POST" id="editTextureForm">
                        @csrf 
                        @method('PUT')
                                    ...
                                    <div class="d-flex gap-3 mt-2">
                                        <div class="form-check">
                                            <input class="form-check-input" type="radio" name="status" id="active" value="1"
                                                {{ $texture->status == 1 ? 'checked' : '' }}>
                                            <label class="form-check-label" for="active">Hoạt động</label>
                                        </div>
                                        <div class="form-check">
                                            <input class="form-check-input" type="radio" name="status" id="inactive" value="0"
                                                {{ $texture->status == 0 ? 'checked' : '' }}>
                            ...
                            <div class="col-12">
                                <div class="mb-3">
                                    ...
                                    <textarea name="description" id="description" class="form-control" rows="3" 
                                              placeholder="Nhập mô tả cho chất liệu...">{{ old('description', $texture->description) }}</textarea>
                                    @error('description')
                                        <div class="text-danger">{{ $message }}</div>
                                    @enderror
                                </div>

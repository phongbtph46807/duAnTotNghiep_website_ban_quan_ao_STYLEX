...
<body>
    <div class="verification-container">
        @if(strpos($msg, 'thành công') !== false || strpos($msg, 'success') !== false)
            <div class="verification-icon">
                <i class="ri-checkbox-circle-line text-success"></i>
            </div>
            <h3 class="verification-title text-success">{{ $msg }}</h3>
            <p class="verification-message">Bạn có thể đăng nhập vào tài khoản của mình ngay bây giờ.</p>
            <a href="{{ route('loginView') }}" class="btn btn-primary btn-lg">
        ...
        @else
            ...
            <h3 class="verification-title text-warning">{{ $msg }}</h3>
            <p class="verification-message">Vui lòng kiểm tra email và thử lại với link mới.</p>
            <div class="d-flex gap-2 justify-content-center">
                <a href="{{ route('registerView') }}" class="btn btn-outline-secondary">
                    <i class="ri-arrow-left-line me-2"></i> Quay lại đăng ký
                </a>
                <a href="{{ route('loginView') }}" class="btn btn-primary">
